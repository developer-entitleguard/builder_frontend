-- Restructure user/organization system (Fixed version)
-- Organizations are primary, each has a contact user, organizations can add multiple users

-- First, drop all dependent policies that reference organization_id in profiles
...
-- Drop other organization policies that need updating
...
BEGIN
  ...
  -- If user doesn't have a role, they might be a new organization contact
  -- This will be handled separately in the application logic
END;
$$;